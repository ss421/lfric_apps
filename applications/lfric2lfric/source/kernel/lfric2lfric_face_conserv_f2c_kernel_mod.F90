...
!> @brief The weighted conservative restriction operation for scalar face fields.
...
module lfric2lfric_face_conserv_f2c_kernel_mod
...
type, public, extends(kernel_type) :: lfric2lfric_face_conserv_f2c_kernel_type
   private
...
end type lfric2lfric_face_conserv_f2c_kernel_type

public :: lfric2lfric_face_conserv_f2c_kernel_code

  ! Generic interface for real32 and real64 types
  interface lfric2lfric_face_conserv_f2c_kernel_code
    module procedure                                                          &
      lfric2lfric_face_conserv_f2c_code_r_single,                             &
      lfric2lfric_face_conserv_f2c_code_r_double
  end interface
...
contains
  ...
  !!                                         restriction. When having multidata
  !!                                         fields the actual dimension of this
  !!                                         field will be smaller than the
  !!                                         default and care needs to be taken
  ...
  ! R_SINGLE PRECISION
  ! ==================
  subroutine lfric2lfric_face_conserv_f2c_code_r_single(                      &
                                                  nlayers,                    &
                                                  cell_map,                   &
        ...
        do m = 0, multidata
          do k = 0, top_df
            coarse_field(map_coarse(df)+m*(top_df+1)+k) = &
              ...
              denom * weights((map_fine(df,cell_map(x_idx,y_idx))-1)/(multidata+1)+k+1)
          end do
        end do
      end do
    end do

  end subroutine lfric2lfric_face_conserv_f2c_code_r_single

  ! R_DOUBLE PRECISION
  ! ==================
  subroutine lfric2lfric_face_conserv_f2c_code_r_double(                      &
                                                  nlayers,                    &
                                                  cell_map,                   &
                                                  ncell_fine_per_coarse_x,    &
                                                  ncell_fine_per_coarse_y,    &
  ...
  end subroutine lfric2lfric_face_conserv_f2c_code_r_double

end module lfric2lfric_face_conserv_f2c_kernel_mod
